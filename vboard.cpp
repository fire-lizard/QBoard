...
void VBoard::paintEvent(QPaintEvent *)
{
    ...
    case EuroShogi:
		...
		break;
    case WhaleShogi:
        resourcePrefix = ":/pieces_wha/images_wha/";
        break;
    case ToriShogi:
        ...
        break;
    case YariShogi:
        if (_pieceStyle == European) resourcePrefix = ":/pieces_yam/images_yam/";
        else if (_pieceStyle == Mnemonic) resourcePrefix = ":/pieces_ptg/images_ptg/";
        else resourcePrefix = ":/pieces_yam/images_yam/";
        break;
    case ChuShogi:
	...
	for (int i = 0; i < _board->GetWidth(); i++)
	{
		for (int j = 0; j < _board->GetHeight(); j++)
		{
            ...
            else if ((_gameVariant == Chess || _gameVariant == CapablancaChess || _gameVariant == GothicChess || _gameVariant == JanusChess) &&
...
void VBoard::contextMenuEvent(QContextMenuEvent* event)
{
	...
	if (_editorMode)
	{
		...
		if (selectedAction != nullptr)
		{
            ...
            {
                _chosenPiece = WhaleShogiPiece::Description2PieceType(selectedAction->text().toStdString());
            }
            else if (_gameVariant == ToriShogi)
            {
            ...
            {
                _chosenPiece = YariShogiPiece::Description2PieceType(selectedAction->text().toStdString());
            }
	...
	if (selectedAction != nullptr)
	{
        ...
        PieceType newPiece;
        if (_gameVariant == WhaleShogi)
            newPiece = WhaleShogiPiece::Description2PieceType(longStringCode);
        else if (_gameVariant == ToriShogi)
            newPiece = ToriShogiPiece::Description2PieceType(longStringCode);
        else if (_gameVariant == YariShogi)
            newPiece = YariShogiPiece::Description2PieceType(longStringCode);
        else if (_gameVariant == CrazyWa)
