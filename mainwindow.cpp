...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
	...
	if (QFile::exists(_settingsDir + "/" + _settingsFileName))
	{
		...
		this->ui->vboard->SetPieceStyle(EngineManager::StringToPieceStyle(settings.pieceStyle));
...
void MainWindow::on_actionSettings_triggered()
{
	...
	if (settingsDialog->result() == QDialog::Accepted)
	{
		...
		const PieceStyle pieceStyle = EngineManager::StringToPieceStyle(settingsDialog->GetGamePieces()->currentText());
...
{
	QString aboutStr;
    aboutStr.append("<center>QBoard 1.2.0.1<br/>");
    ...
    aboutStr.append("2017 - 2026</center>");
		...
		gameVariant == Shatar || gameVariant == Sittuyin)
	{
		...
		if (fileDialog.exec())
		{
			...
			else if (fileDialog.selectedNameFilter() == "PGN Files (*.pgn)")
			{
                ...
                else if (gameVariant == GothicChess)
                    chessVariant = "[Variant \"gothic\"]\n\n";
                else if (gameVariant == JanusChess)
