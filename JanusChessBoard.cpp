...
void JanusChessBoard::GetMoves(const std::optional<Piece>& piece, int x, int y)
{
    _moves.clear();
    switch (piece->Type)
    {
    case King:
        ...
        if (!piece->HasMoved)
        {
            ...
            if (GetData(9, y) != std::nullopt)
            {
                ...
                if (!cp->HasMoved && cp->Type == Rook && GetData(5, y) == std::nullopt && GetData(6, y) == std::nullopt && GetData(7, y) == std::nullopt && GetData(8, y) == std::nullopt)
                {
