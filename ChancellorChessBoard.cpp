...
void ChancellorChessBoard::GetMoves(const std::optional<Piece>& piece, int x, int y)
{
    _moves.clear();
    switch (piece->Type)
    {
...
bool ChancellorChessBoard::Move(int oldX, int oldY, int newX, int newY, bool cl)
{
    ...
    if (result && GetData(newX, newY) != std::nullopt)
    {
        ...
        else if (pieceType == Pawn && _ep != "-")
        {
            ...
            {
                const std::optional<Piece> p = pieceColour == White ? GetData(newX, number) : GetData(newX, number + 2);
                if (p != std::nullopt && p->Type == Pawn && p->Colour != pieceColour)
                {
                    ...
                    {
                        SetData(newX, number + 2, std::nullopt);
                    }
                }
            }
            _ep = "-";
